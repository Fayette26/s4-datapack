...
execute if score message dingtimer matches 17 run tellraw @a[tag=!muteding] ["",{color:"white",text:"日 "},{color:"#f5e6a5",text:"The spring event is now live! Have some free tokens!"}]
execute if score message dingtimer matches 17 minecraft:give @s paper[custom_model_data={floats:[25]},item_name={"bold":true,"color":"yellow","text":"Flower Token"},lore=[{"color":"dark_purple","text":"Can be traded for cosmetics at the Flower Festival"}]] 1
